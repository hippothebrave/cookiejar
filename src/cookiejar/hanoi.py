...
def main():
    ...
    while True:  # Run a single turn on each iteration of this loop.
        ...
        if SOLVED_TOWER in (towers["B"], towers["C"]):
            ...
            print(f"You used {movecounter} moves. The minimum is {2**TOTAL_DISKS - 1}.")
            sys.exit()


def getPlayerMove(towers):
    """Ask the player for a move until you get a valid response, or QUIT."""
    while True:  # Keep asking the player until you get a valid move.
        ...
        if response not in ("AB", "AC", "BA", "BC", "CA", "CB"):
            print("Moves must be one of: AB, AC, BA, BC, CA, CB.")
            continue  # keep asking for move

        from_tower, to_tower = response[0], response[1]

        if len(towers[from_tower]) == 0:
            print("You cannot move disks from an empty tower.")
            continue  # keep asking for move
            ...
            print("Smaller disks cannot go on top of larger disks.")
            continue  # keep asking for move
        else:
            return from_tower, to_tower
...
def displayTowers(towers):
    ...
    print("{0} A{0}{0} B{0}{0} C\n".format(emptySpace)) # tower base = 2: [space][letter]
